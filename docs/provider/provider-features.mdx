...
```
#### Extension - config
since extensions must offer consumers the regular spec and the possibility of an extensions, both must be present.
...
if you've set up the second option, meaning one provider service per multiple nodes, it is required to provide stickyness across the nodes by a header

the reason for this being the cryptographic proofs a provider signs, must be consistent and can't have the blocks progress backwards 
