...
Once a provider is up and running, consumers will request relay services from the provider. once the provider provide the service he will be aligible to get rewards from the consumer he served (this happens when a consumers mopnthly plan expires).

providers can Query the estimated amount they will recieve with the query:
